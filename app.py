...
def index():
    ...
    llm_status = {
        ...
        'deployment': llm_parser.get_active_engine() if llm_parser.is_available() else None
    }
...
def process_files_background(session_id, uploaded_files):
    ...
    try:
        ...
        for cluster_index, cluster in enumerate(all_clusters):
            ...
            # Use LLM/Jev parser to analyze the cluster
            llm_result = llm_parser.analyze_birthday_cluster(cluster, cluster_messages, all_participants)
...
def api_identity_detail(identity_id):
    ...
    try:
        ...
        result['confidence_explanation'] = {
            'simplified': True,
            'note': 'Using simplified confidence calculation'
        }
...
def export_ics():
    """Export selected birthdays as ICS calendar file."""
    try:
        data = request.get_json()
        birthdays = data.get('birthdays', [])
        
        if not birthdays:
            return jsonify({'error': 'No birthdays selected'}), 400
        
        # Generate ICS content
        ics_content = generate_ics_calendar(birthdays)
        ...
        logger.info(f"Generated ICS file for {len(birthdays)} birthdays")
        return response
...
def generate_ics_calendar(birthdays):
    ...
    ics_lines = [
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
        "PRODID:-//WhatsApp Birthday Predictor//Birthday Events//EN",
        "CALSCALE:GREGORIAN",
        "METHOD:PUBLISH",
        "X-WR-CALNAME:Birthdays from WhatsApp",
        "X-WR-CALDESC:Birthday reminders extracted from WhatsApp chats"
    ]
    ...
    for birthday in birthdays:
        ...
        day = birthday['day']
        confidence = birthday.get('confidence', 0) * 100
        ...
        ics_lines.extend([
            ...
            f"DESCRIPTION:Birthday reminder for {name} (Confidence: {confidence:.0f}%)\\n\\nGenerated by WhatsApp Birthday Predictor",
            "RRULE:FREQ=YEARLY",
